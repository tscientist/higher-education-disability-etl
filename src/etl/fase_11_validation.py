"""
Fase 11: Validation and Test Output
...
class Fase11Validation:
    ...
    def run(self, fase_results):
        """
        Executa validação e exibe resultados.
        
        Args:
            fase_results: Dicionário com resultados de cada fase
        ...
        try:
            ...
            stats = {
                "extraction": fase_results.get("fase_1", {}),
                "sisu": {
                    "agregated_groups": len(fase_results.get("sisu_aggregations", {})),
                    "final_docs": len(fase_results.get("sisu_final_docs", [])),
                },
                "final": {
                    "total_docs": len(fase_results.get("final_documents", [])),
                    "docs_with_sisu": sum(
                        1 for d in fase_results.get("final_documents", [])
                        if d.get("sisu", {}).get("hasMatch")
                    ),
                },
                "mongodb": fase_results.get("mongodb_stats", {}),
            }
