...
that the service account or local Google Cloud authentication has proper access.
...
# Optional: set GOOGLE_APPLICATION_CREDENTIALS only if GCP_CREDENTIALS_PATH is provided.
#
# If GCP_CREDENTIALS_PATH is empty, the script will use Application Default Credentials,
# which can be configured locally with:
#
...
if credentials_path and not os.getenv("GOOGLE_APPLICATION_CREDENTIALS"):
    ...
    if not credentials_path.is_absolute():
        credentials_path = Path(__file__).parent.parent.parent / credentials_path
...
def get_bigquery_client():
    ...
    Uses either:
    - GOOGLE_APPLICATION_CREDENTIALS, when a service account file is configured; or
...
def create_dataset(client, dataset_id):
    ...
    Args:
        ...
        dataset_id (str): Dataset ID, for example 'ppgti_etl_test'
...
def create_test_table(client, dataset_id, table_id):
    ...
    Args:
        ...
        table_id (str): Table ID, for example 'etl_test_table'
